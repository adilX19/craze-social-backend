...
def get_followers_graph_data(followers_graph_data):
    ...
    for follower_data in followers_graph_data:
        key = list(follower_data.keys())[0]
        ...
        followers_gr = follower_data.get(key)
        for label in labels:

            try:
                gr = followers_gr[labels.index(label)]
            except:
                gr = {
                    "month": label,
                    "followers": 0,
            ...
            if gr.get("month")[:3] in labels:
                results_data['data'].append(int(gr.get("followers", 0)))
            else:
...
def get_videos_or_normal_posting_data(posting_data):
    cleaned_data = []
    labels = ['Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct']
    ...
    for data in posting_data:
        ...
        for label in labels:
            ...
            if gr.get("month") in labels:
                results_data['data'].append(int(gr.get("posts", 0)))
            else:
                results_data['data'].append(0)
