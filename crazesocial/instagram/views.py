...
from instagram.models import InstagramProfileInsights, InstagramUserProfile
...
def insta_extraction_data(request):
    search_activity = UserActivity.objects.filter(flow_name='Instagram').last()
    
    response = requests.get('http://66.94.109.65/instagram/crazysocial/insights/', params={
    ...
    customer_data["profile_data"]["class"] = classify_insta_profile(
        ...
        customer_data.get("profile_data", {}).get("followersCount", 0),
...
def insta_data_save_db(request):
    
    activity_object = UserActivity.objects.filter(flow_name='Instagram').first()
...
def instagram_data_response(request):
    competitors_data_list = []
    followers_graph_data = []
    followers_gained_per_week_data = []
    ...
    search_activity = UserActivity.objects.filter(django_user=request.user.username, flow_name='Instagram').last()

    # CUSTOMER DATA FETCHING FROM DATABASE...
    customer_profile = InstagramUserProfile.objects.get(
        insta_userhandle=search_activity.customer_handle,
        created=search_activity.created
    )
    customer_insights = InstagramProfileInsights.objects.get(
        insta_userhandle=search_activity.customer_handle,
        created=search_activity.created
    )
    ...
    for competitor_handle in comp_handles_join:
        ...
        if '---common---' in competitor_handle:
            common_search_activity = UserActivity.objects.filter(
                django_user=competitor_handle.split('---')[-1], flow_name='Instagram').last()
            created = common_search_activity.created
            competitor_handle = competitor_handle.split('---')[0]
        else:
            created = search_activity.created

        competitor_profile = InstagramUserProfile.objects.get(
            insta_userhandle=competitor_handle,
            created=created
        )
        competitor_insights = InstagramProfileInsights.objects.get(
            insta_userhandle=competitor_handle,
            created=created
        )
        ...
        competitor_wordcloud_data = get_wordcloud_data(customer_profile.wordcloud_string)

        competitor_profile = model_to_dict(competitor_profile)
        del competitor_profile["wordcloud_string"]

        competitor_insights = model_to_dict(competitor_insights)
        competitor_insights["posts_engagement"] = float(competitor_insights["posts_engagement"])
        competitor_insights["paid_posts_engagement"] = float(competitor_insights["paid_posts_engagement"])
        competitor_insights["reels_engagement"] = float(competitor_insights["reels_engagement"])

        competitor_insights["posting_growth"] = float(competitor_insights["posting_growth"])
        competitor_insights["followers_growth"] = float(competitor_insights["followers_growth"])
        competitor_insights["following_growth"] = float(competitor_insights["following_growth"])

        competitors_data_list.append({
            'profile_data': competitor_profile,
            'insights_data': competitor_insights,
    ...
    try:
        mining_response = requests.get("http://66.94.109.65/instagram/profile_analytics/", params={
    ...
    customer_insights["posts_engagement"] = float(customer_insights["posts_engagement"])
    customer_insights["paid_posts_engagement"] = float(customer_insights["paid_posts_engagement"])
    customer_insights["reels_engagement"] = float(customer_insights["reels_engagement"])
    ...
    customer_insights["best_engagement_post_type"] = customer_analytics.get("best_engagement_post_type", {})
    customer_insights["pictures_post_in_month"] = customer_analytics.get("pictures_post_in_month", [])
    customer_insights["videos_post_in_month"] = customer_analytics.get("videos_post_in_month", [])
    ...
    # MONTHLY FOLLOWERS GRAPH DATA GATHERING FOR CUSTOMER PROFILE...
    followers_graph_data.append({
        f'{search_activity.customer_handle}_followers': customer_analytics.get("followers_gained_per_month", [])
    })

    # WEEKLY FOLLOWERS GRAPH DATA GATHERING FOR CUSTOMER PROFILE...
    followers_gained_per_week_data.append({
        f'{search_activity.customer_handle}_followers': customer_analytics.get("followers_gained_per_week", [])
    })
    ...
    # WEEKLY INTERACTIONS DATA OF CUSTOMER...
    ...
    # VIDEOS POSTING DATA OF CUSTOMER...
    videos_posting_data.append({
        f'{search_activity.customer_handle}_videos': customer_analytics.get("videos_post_monthly", [])
    })

    # NORMAL POSTING DATA OF CUSTOMER...
    normal_posting_data.append({
        f'{search_activity.customer_handle}_normal': customer_analytics.get("normal_post_monthly", [])
    })
    ...
    for analytics in competitors_analytics:
        ...
        competitor_data["insights_data"]["followers_gained_per_month"] = analytics.get("followers_gained_per_month", [])
    # ===================================================================================================

        # MONTHLY FOLLOWERS GRAPH DATA GATHERING FOR COMPETITORS PROFILES...
        followers_graph_data.append({
            f'{competitor_data["profile_data"]["insta_userhandle"]}_followers': analytics.get("followers_gained_per_month", [])
        })

        # WEEKLY FOLLOWERS GRAPH DATA GATHERING FOR COMPETITORS PROFILES...
        followers_gained_per_week_data.append({
            f'{competitor_data["profile_data"]["insta_userhandle"]}_followers': analytics.get("followers_gained_per_week", [])
        })

        # BEST MONTHLY ENGAGEMENT DATA FOR COMPETITORS PROFILES...
        best_monthly_engagement_data.append({
            f'{competitor_data["profile_data"]["insta_userhandle"]}_engagements': analytics.get("best_monthly_engagement", [])
        })
        
        # BEST WEEKLY ENGAGEMENT DATA FOR COMPETITORS PROFILES...
        best_weekly_engagement_data.append({
            f'{competitor_data["profile_data"]["insta_userhandle"]}_engagements': analytics.get("best_weekly_engagement", [])
        })

        # MONTHLY INTERATIONS DATA OF COMPETITORS PROFILES...
        monthly_interactions_data.append({
            f'{competitor_data["profile_data"]["insta_userhandle"]}_interactions': analytics.get("monthly_interactions", [])
        })

        # WEEKLY INTERACTIONS DATA OF COMPETITORS PROFILES...
        weekly_interactions_data.append({
            f'{competitor_data["profile_data"]["insta_userhandle"]}_interactions': analytics.get("weekly_interactions", [])
        })

        # VIDEOS POSTING DATA OF COMPETITORS PROFILES...
        videos_posting_data.append({
            f'{competitor_data["profile_data"]["insta_userhandle"]}_videos': analytics.get("videos_post_monthly", [])
        })

        # NORMAL POSTING DATA OF COMPETITORS PROFILES...
        normal_posting_data.append({
            f'{competitor_data["profile_data"]["insta_userhandle"]}_normal': analytics.get("normal_post_monthly", [])
        })
    ...
    customer_profile["radial_chart"] = {
        ...
        'data': [int(customer_profile['followersCount']), float(customer_insights['posts_engagement']), float(customer_insights["often_posting_month"][:2])]
    }
    # ===================================================================================================
    ...
    followers_table = [{
        'user_handle': customer_profile["insta_userhandle"],
    ...
    for data in competitors_data_list:
        ...
        followers_table.append({
            'user_handle': competitor_profile["insta_userhandle"],
    ...
    return Response({
        "dataset": {
            ...
            "followers_gained_monthly": get_followers_graph_data(followers_graph_data),
            "followers_gained_weekly": get_weekly_distribution_of_data(followers_gained_per_week_data, "followers"),
            ...
            "videos_posting_monthly": get_videos_or_normal_posting_data(videos_posting_data),
            "normal_posting_monthly": get_videos_or_normal_posting_data(normal_posting_data)
        }
    })